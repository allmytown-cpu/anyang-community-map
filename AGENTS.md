...
| File | Purpose |
|---|---|
| `index.html` | **Main map** (GitHub Pages). Read-only Leaflet map with hardcoded GPS waypoints (from Excel). Route polyline, numbered markers. |
...
- **exif-js 2.3.0** — EXIF GPS extraction (upload file only)
...
- **Storage**: `community_map_upload.html` uses `window.storage` (Netlify Blobs or similar) with keys `photo_index` (JSON array of IDs) and `photo:{id}` (JSON records). If this API is unavailable, the upload flow will fail silently.
...
- The Excel file is not read programmatically — data in `community_map_prototype.html` is hardcoded. If you update the Excel, you must manually sync the `points` array.
